...
users: Manages authentication and session persistence, storing the id, username, password_hash, and a session_token[cite: 2]. It also includes a must_change_password boolean flag for initial account setup[cite: 2].

conversations: Tracks individual chat sessions, storing the id, user_id, and title[cite: 2]. It utilizes created_at and updated_at timestamps for sorting chat history[cite: 2].

messages: Links directly to conversations via a foreign key constraint and stores the id, conversation_id, role, and content[cite: 2]. It caches the dynamically generated TTS links using audio_url and audio_url_2 so the application does not need to re-fetch identical audio files[cite: 2].
